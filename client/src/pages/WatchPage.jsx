...
import { formaReleaseDate } from '../utils/dateFunction'
...
const WatchPage = () => {
    ...
    useEffect(() => {
        const getTrailers = async () => {
            setLoading(true)
            try {
                const response = await axios.get(`/v1/${contentType}/${id}/trailers`)
    ...
    useEffect(() => {
        const getSimilarContent = async () => {
            try {
                const response = await axios.get(`/v1/${contentType}/${id}/similar`)
    ...
    useEffect(() => {
        const getContentDetails = async () => {
            try {
                const response = await axios.get(`/v1/${contentType}/${id}/details`)
    ...
    return (
        <div className="bg-black min-h-screen text-white">
            <div className="mx-auto container px-4 py-8 h-full">
                ...
                ${currentTrailerIdx === 0 ? "opacity-50 cursor-not-allowed hover:bg-gray-500/70" : ""}`}
                        ...
                        <button
                            className={`bg-gray-500/70 hover:bg-gray-500 text-white py-2 px-4 rounded
                ${currentTrailerIdx === trailers.length - 1 ? "opacity-50 cursor-not-allowed hover:bg-gray-500/70" : ""}`}
                ...
                >
                    <div className="mb-4 md:mb-0">
                        ...
                        <p className="mt-2 text-lg">
                            {formaReleaseDate(content?.release_date || content?.first_air_date)} |{" "}
                            {content?.adult ? (
